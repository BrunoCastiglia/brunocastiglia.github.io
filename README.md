# MoneyToGo — brunocastiglia.github.io
...
🔗 **[Abrir o site](https://brunocastiglia.github.io/)**
...
Este repositório é o **`brunocastiglia.github.io`**, então o MoneyToGo é servido
na **raiz do domínio** — `https://brunocastiglia.github.io/`, não numa subpasta.
Isso não é detalhe de organização: para o AdSense o "site" é o domínio inteiro, e
quem revisa entra pela raiz. Enquanto o app morava em `/moneytogo/`, a raiz era
uma página-vitrine de três linhas, que é o retrato de "conteúdo de baixo valor"
que reprova a inscrição. O repositório `moneytogo` continua existindo, mas só
com um redirecionamento para cá, para não haver duas cópias do mesmo site no
mesmo domínio.

Todos os caminhos do código são relativos, então nada precisa mudar quando um
domínio próprio for apontado para cá.

### O `ads.txt`

O Google procura o arquivo em `https://brunocastiglia.github.io/ads.txt`, e só
ali — a raiz do domínio. É por isso que ele vive neste repositório:
...
publishers; o que muda é só o `pub-`. O Google leva de algumas horas a um dia
para rastrear o arquivo depois de publicado.
